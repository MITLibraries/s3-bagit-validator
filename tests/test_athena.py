...
class TestAthenaClientInitialization:
    def test_get_sqlalchemy_engine_created_from_config_success(self):
...
class TestAthenaClientQueries:
    def test_perform_query_returns_dataframe(
        self, mocked_engine, mocked_pandas_read_sql_query  # noqa: ARG002
    ):
        athena_client = AthenaClient()
...
class TestAthenaIntegration:
    ...
    def test_sqlalchemy_engine_connection_performs_query_success(self):
    ...
    @pytest.mark.integration
    def test_sqlalchemy_engine_connection_bad_workgroup_throws_error(self, monkeypatch):
